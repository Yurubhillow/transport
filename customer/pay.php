    ...
    <section class="main-dashboard">
        ...
        <div class="right-bar">
            ...
            <div class="payment-form">
                <form action="backend/payments.php" method="post">
                    <label for="">Total Amount to be paid</label>
                    <?php 
                       $cargo_id = $_REQUEST['cargo_id'];
                       $selectcargo = mysqli_query($conn,"select * from customer join cargo on customer.customer_id=cargo.customer_id join payment on cargo.cargo_id=payment.cargo_id where company_email='$email' and payment.cargo_id='$cargo_id'");
                       $rowcargo = mysqli_fetch_array($selectcargo);
                    ?>
                    <input type="text" value="<?php echo $rowcargo['balance'];?>" name="balances" readonly><br>
                    <br>
                    <label for="">Total amount paid</label>
                    <input type="number" placeholder="Enter Total Amount paid" name="paid" required><br>
                    <br>
                    <label for="">Mpesa Ref Number</label>
                    <input type="text" placeholder="Enter Confirmation Number" name="m_ref" required><br>
                    <br>
                    <input type="hidden" value="<?php echo $rowcargo['paid_amount'];?>" name="ppay">
